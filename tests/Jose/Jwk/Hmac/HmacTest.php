...
use IdentityLayer\Jose\Jwk\Hmac\Hmac256;
...
class HmacTest extends TestCase
{
    public function testSignatureGenerationWithAllHmacVariants()
    {
        ...
        $jwk = Hmac256::fromKidAndKey('1', 'y:¹£ö;"T.Gvš×äâÖåUgøé!œ¡%ôÍ');
        $this->assertEquals(
            $expectedSignature,
            Base64UrlSafe::encodeUnpadded(
                $jwk->sign(AlgorithmName::HS256(), $message256)
            )
        );
        ...
        $jwk = Hmac256::fromKidAndKey('1', 'ⵇ頞샴揎庋騒䦘뎌誾﯎̀ቲ谀຅䘖꺹ㄏඞ嚙鶲⑐澤');
        $this->assertEquals(
            $expectedSignature,
            Base64UrlSafe::encodeUnpadded(
                $jwk->sign(AlgorithmName::HS384(), $message384)
            )
        );
        ...
        $jwk = Hmac256::fromKidAndKey('1', '降驱쓑በꠊ翟溊㊓툀ᇐ霋㔀΍捜뮨ʑ䦻ᔱ铗ꦛ枩鷠烀矾鮟ꥏ鹿凯궑鐍');
        $this->assertEquals(
            $expectedSignature,
            Base64UrlSafe::encodeUnpadded(
                $jwk->sign(AlgorithmName::HS512(), $message512)
            )
        );
    }
}